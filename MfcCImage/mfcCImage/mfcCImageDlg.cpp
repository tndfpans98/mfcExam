...
CString g_strFileImage = (_T("C:\\Users\\이영진\\Desktop\\2image\\save.bmp"));
...
void CmfcCImageDlg::moveRect()
{
	...
	DrawCircle(fm, nSttX, nSttY, nRad, nGray);
	//for (int j = nSttY; j < nSttY + 48; j++)
	//{
		//for (int i = nSttX; i < nSttX + 64; i++)
		//{
			//drawCircle(fm, i, j, nRad, nGray);
			//if(validImgPos(i, j))
			//	fm[j * nPitch + i] = nGray;
		//}
	//}
...
void CmfcCImageDlg::DrawCircle(unsigned char* fm, int x, int y, int nRad, int nGray)
{
	int nX = x + nRad;
	int nY = y + nRad;
	int nPitch = m_image.GetPitch();
	for (int j = y; j < y + nRad * 2; j++)
	{
		for (int i = x; i < x + nRad * 2; i++)
		{
			if(isInCircle(i, j, nX, nY, nRad))
			fm[j * nPitch + i] = nGray;
		}
	}
}

bool CmfcCImageDlg::isInCircle(int i, int j, int nX, int nY, int nRad)
{
	bool bRet = false;
	double dX = i - nX;
	double dY = j - nY;
